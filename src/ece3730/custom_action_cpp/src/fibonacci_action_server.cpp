...
public:
  ...
  : Node("fibonacci_action_server", options)
  {
    ...
    auto handle_goal = [this]( //Got request
      const rclcpp_action::GoalUUID & uuid,
      std::shared_ptr<const Fibonacci::Goal> goal)
    {
      RCLCPP_INFO(this->get_logger(), "Received goal request with order %d", goal->order);
      (void)uuid;
      return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE; //client is told that the goal was accepted
    };

    auto handle_cancel = [this]( //request cancelled
    ...
    auto handle_accepted = [this]( //handle request with goal identifier below
      const std::shared_ptr<GoalHandleFibonacci> goal_handle) 
    {
